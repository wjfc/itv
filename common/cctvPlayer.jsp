...
<script type="text/javascript">
    ...
    function initPage() {
        getMovieInfo();
        //Action.chsLog(CHS.STB.getStbNo());
    }

    var queryPriceTime = null;
    function getMovieInfo() {
        var url = "../cctvMovie/detail.do?categoryId=${param.categoryId}&assetId=${param.assetId}";
    ...
    function getProgress() {
        if(Action.browType=="DVN"){ //天柏（主要适配苏州 和无锡）
            ...
            var speed = speedArr[speedPos];
            statusShow(curStatus, speed);   //设置控制条状态

            if(curStatus >= 2){
                //Action.chsLog("  curStatus="+curStatus+"  speed="+speed+" getCurrentTime()="+getCurrentTime()+"  ");
            }
            ...
            if (curStatus >= 2) {
                ...
                if(refreshNum>1&&initFlag){   //此判断是为了防止 三秒后 提示框 没有消失的情况 强制消失
                    hideVideoTip();
                    showProgress();
                    initFlag = false;
                }
    ...
    function eventHandle(keyCode) {
        ...
        switch (keyCode) {
            ...
            case Action.KEY.EXIT:
                ...
                break;
            case Action.KEY.END:
            case Action.KEY.END1:
            case Action.KEY.END2:
